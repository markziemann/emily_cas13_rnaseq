...
pdf("dsred_hf_box.pdf",height=5,width=5)
boxplot(dat,ylab="Reads per million",ylim=c(0,MAX),
  main="DsRed expresssion (Hf)", col="white",frame=FALSE)
beeswarm(dat,add=TRUE,col="red",cex=2,pch=19)
mtext(paste("Reduction:",reduction,"%"))
...
pdf("dsred_rfx_box.pdf",height=5,width=5)
boxplot(dat,ylab="Reads per million",ylim=c(0,MAX),
  main="DsRed expresssion (Rfx)", col="white",frame=FALSE)
beeswarm(dat,add=TRUE,col="red",cex=2,pch=19)
mtext(paste("Reduction:",reduction,"%"))
...
pdf("gfp_hf_box.pdf",height=5,width=5)
boxplot(dat,ylab="Reads per million",ylim=c(0,MAX),
  main="Fbxw11-GFP expresssion (Hf)", col="white",frame=FALSE)
beeswarm(dat,add=TRUE,col="red",cex=2,pch=19)
mtext(paste("Reduction:",reduction,"%"))
...
pdf("gfp_rfx_box.pdf",height=5,width=5)
boxplot(dat,ylab="Reads per million",ylim=c(0,MAX),
  main="Fbxw11-GFP expresssion (Rfx)", col="white",frame=FALSE)
beeswarm(dat,add=TRUE,col="red",cex=2,pch=19)
mtext(paste("Reduction:",reduction,"%"))
...
make_volcano2 <- function(de,name) {
    de <- de[!is.na(de$padj),]
    de <- de[grep("Fbxw11-GFP",rownames(de),invert=TRUE),]
    de <- de[grep("DsRed",rownames(de),invert=TRUE),]
    sigup <- subset(de,padj<0.05 & log2FoldChange > 0.5)
    sigdn <- subset(de,padj<0.05 & log2FoldChange < -0.5)
    N_UP <- nrow(sigup)
    N_DN <- nrow(sigdn)
    N_SIG <- N_UP + N_DN
    DET=nrow(de)
    HEADER=paste(DET, "genes detected,",  N_SIG,"@5%FDR")
    plot(de$log2FoldChange,-log10(de$padj),cex=0.6,pch=19,col="darkgray",bty ="n",
        main=name, xlab="log2 FC", ylab="-log10 FDR")
    mtext(HEADER)
    abline(v=0.5,lwd=1,lty=2)
    abline(v=-0.5,lwd=1,lty=2)
    abline(h=1.301,lwd=1,lty=2)
    points(sigup$log2FoldChange,-log10(sigup$padj),cex=0.6,pch=19,col="darkorange")
    points(sigdn$log2FoldChange,-log10(sigdn$padj),cex=0.6,pch=19,col="deepskyblue")
    YPOS=max(-log10(de$padj[!is.na(de$padj)]))
    XPOS_DN=min(de$log2FoldChange)/2
    XPOS_UP=max(de$log2FoldChange)/2
    text(x=XPOS_UP,y=YPOS,labels=N_UP)
    text(x=XPOS_DN,y=YPOS,labels=N_DN)
}
...
make_volcano2(dge1,"Rfx: NTC gRNA vs DsRed gRNA")
pdf("dge1.pdf",height=5,width=5)
make_volcano2(dge1,"Rfx: NTC gRNA vs DsRed gRNA")
dev.off()
...
make_volcano2(dge2,"Rfx: Tfx ctl vs NTC gRNA")
pdf("dge2.pdf",height=5,width=5)
make_volcano2(dge2,"Rfx: Tfx ctl vs NTC gRNA")
dev.off()
...
make_volcano2(dge3,"Rfx: NTfc ctl vs Tfx ctl")
pdf("dge3.pdf",height=5,width=5)
make_volcano2(dge3,"Rfx: NTfc ctl vs Tfx ctl")
dev.off()
...
make_volcano2(dge4,"Hf: NTC gRNA vs DsRed gRNA")
pdf("dge4.pdf",height=5,width=5)
make_volcano2(dge4,"Hf: NTC gRNA vs DsRed gRNA")
dev.off()
...
head(mres4$enrichment_result,20) %>%1
...
make_volcano2(dge5,"Hf: Tfx ctl vs NTC gRNA")
pdf("dge5.pdf",height=5,width=5)
make_volcano2(dge5,"Hf: Tfx ctl vs NTC gRNA")
dev.off()
...
make_volcano2(dge6,"Hf: NTfc ctl vs Tfx ctl")
pdf("dge6.pdf",height=5,width=5)
make_volcano2(dge6,"Hf: NTfc ctl vs Tfx ctl")
dev.off()
...
make_volcano2(dge7,"Rfx DsRed gRNA vs Hf DsRed gRNA")
pdf("dge7.pdf",width=5,height=5)
make_volcano2(dge7,"Rfx DsRed gRNA vs Hf DsRed gRNA")
dev.off()
...
make_volcano2(dge8,"Rfx NTC gRNA vs Hf NTC gRNA")
pdf("dge8.pdf",width=5,height=5)
make_volcano2(dge8,"Rfx NTC gRNA vs Hf NTC gRNA")
dev.off()
...
make_volcano2(dge9,"Rfx Tfx ctl vs Hf Tfx ctl")
pdf("dge8.pdf",width=5,height=5)
make_volcano2(dge9,"Rfx Tfx ctl vs Hf Tfx ctl")
dev.off()
...
make_volcano2(dge10,"Rfx NTfx ctl vs Hf NTfx ctl")
pdf("dge10.pdf",width=5,height=5)
make_volcano2(dge10,"Rfx NTfx ctl vs Hf NTfx ctl")
dev.off()
...
make_volcano2(dge11,"gRNA target * Rfx/HF interaction")
pdf("dge11.pdf",width=5,height=5)
make_volcano2(dge11,"gRNA target * Rfx/HF interaction")
dev.off()
...
## Pathway cross comparison

Make a bubble plot of top HF pws (FDR), overlay Rfx.

Then make a bubble plot of top Rfx pws (FDR), overlay HF.

```{r,pwbubble1}

mres1pw <- subset(mres1$enrichment_result, abs(s.dist) > 0.3 & p.adjustANOVA < 0.01)
rowkeep <- c( grep("ribo",mres1pw$set)[1] , grep("ribo",mres1pw$set,invert=TRUE) )
mres1pw <- mres1pw[rowkeep,]
mres1pw <- mres1pw[order(mres1pw$pANOVA),]

mres4pw <- mres4$enrichment_result[which(mres4$enrichment_result$set %in% mres1pw$set),]
mres4pw <- mres4pw[match(mres1pw$set , mres4pw$set),]

ml1 <- as.list(-log10(mres1pw$p.adjustANOVA))
names(ml1) <- mres1pw$set

ml4 <- as.list(-log10(mres4pw$p.adjustANOVA))
names(ml4) <- mres4pw$set

MIN=floor(min(unlist(c(ml1,ml4))))
MAX=ceiling(max(unlist(c(ml1,ml4))))
COLS <- gsub("0","deepskyblue",gsub("2","darkorange",as.character(sign(mres1pw$s.dist)+1)))

par(mar = c(5.1, 20.1, 4.1, 2.1) )

beeswarm(ml1,horiz=TRUE,las=1,xlab="-log10(FDR)",cex=3,pch=19,xlim=c(MIN,MAX),col=COLS)
beeswarm(ml4,horiz=TRUE,cex=3,pch=1,add=TRUE,col=COLS)

legend(15,4.5, inset=.1, title="Construct",
   c("Rfx","HF"), pch=c(19,1), horiz=TRUE, cex=1.0)

legend(15,3.5, inset=.1, title="Direction",
   c("Up","Down"), col=c("deepskyblue","darkorange"), pch=15, horiz=TRUE, cex=1.0)

pdf("pathwaybubbleplot.pdf",width=7,height=5)
par(mar = c(5.1, 20.1, 4.1, 2.1) )
beeswarm(ml1,horiz=TRUE,las=1,xlab="-log10(FDR)",cex=2,pch=19,xlim=c(MIN,MAX),col=COLS)
beeswarm(ml4,horiz=TRUE,cex=2,pch=1,add=TRUE,col=COLS)

legend(19,4.6, inset=.1, title="Construct",
   c("Rfx","HF"), pch=c(19,1), horiz=FALSE, cex=1.0)

legend(19,3.5, inset=.1, title="Direction",
   c("Up","Down"), col=c("deepskyblue","darkorange"), pch=15, horiz=FALSE, cex=1.0)
dev.off()
